...
if (!reproduce) {
  merged <- merge(
    Healthy,
    y = AMLs,
    add.cell.ids = c("reference", "query"),
    project = "abAML"
  )
} else {
  merged <- AMLs
...
patient.metadata <- read.table(
  "Work/dynchro/examples/triana/metadata.tsv",
  sep = "\t",
  header = T,
  row.names = 1
)
...
counts.projection <- normalized_counts[
  intersect(rownames(normalized_counts), rownames(Healthy)),
]

#set up query data
sce_Culture <- SingleCellExperiment(
  assays = list(normcounts = as.matrix(counts.projection))
)
...
sce_All <- SingleCellExperiment(
  assays = list(
    normcounts = as.matrix(Healthy@assays$BOTH@data[
      rownames(counts.projection),
    ])
  )
)
...
library(parallel)
mapped <- mclapply(
  colnames(Culture_Map$sce_All[[1]]),
  Calc,
  cult = Culture_Map$sce_All[[1]],
  mc.cores = 6
)
mapped_ <- mapped
...
options <- rownames(patient.metadata)[patient.metadata$Status == "diseased"] %>%
  as.vector()
...
forcluster <- table(
  as.character(mapped_amls$ct),
  as.character(mapped_amls$sample)
)

#the following line needs to be modified to include only
forcluster <- forcluster[
  apply(forcluster, 1, sum) > 100 &
    !grepl("T cell|NK cell|B cell", rownames(forcluster)),
]
...
bckgr <- data.frame(
  x = Embeddings(Healthy, reduction = "MOFAUMAP")[, 1],
...
qplot(x = x, y = y, data = bckgr, color = I("#BBBBBB"), size = I(0.1)) +
  geom_point(aes(color = dnorm), data = mapped, size = I(0.1)) +
...
getTest <- function(id, assay = "AB") {
  ...
  summed <- sapply(unique(merged$Batch), function(btc) {
    ...
    if (sum(use) == 1)
      ab[, merged$Batch == btc & Idents(merged) == id] else if (sum(use) == 0)
      rep(0, nrow(ab)) else
      apply(ab[, merged$Batch == btc & Idents(merged) == id], 1, sum)
  })
  ncells <- sapply(
    unique(merged$Batch),
    function(btc) sum(merged$Batch == btc & Idents(merged) == id)
  )
  ...
  tryCatch(
    {
      if (runaml) {
        ...
        results <- as.data.frame(results(
          deseq,
          contrast = c("status", "healthy", "diseased")
        ))
...
all.tests.covariate <- lapply(
  unique(as.character(Idents(merged))),
  getTest,
  assay = "AB"
)
...
getRF <- function(ct) {
  ...
  ncells <- sapply(
    unique(AMLs.myeloid$Batch),
    function(btc) sum(AMLs.myeloid$Batch == btc & Idents(AMLs.myeloid) == ct)
  )
  ...
  if (length(unique(pat)) > 1) {
    ...
    data.frame(
      row.names = rownames(rf$importance),
      ct = ct,
      gene = rownames(rf$importance),
      ...
      norm = rf$importance[, "MeanDecreaseAccuracy"] /
        max(rf$importance[, "MeanDecreaseAccuracy"])
    )
  } else NULL
}
...
sig_means <- merge(
  means,
  subset(all.tests.covariate, padj < 0.1 & ct %in% showcelltypes)
)
...
qplot(
  ...
  geom_text_repel(
    size = 3,
    aes(label = gsub("-AB", "", gene)),
    data = subset(sig_means, padj < 0.0001),
    color = "black"
  ) +
  theme_bw() +
  theme(panel.grid = element_blank()) +
  scale_color_gradientn(
    colors = c("grey", "black", "blue", "red", "orange"),
    name = "Inter-patient\nvariability"
  ) +
  scale_size_area(name = "-log10 p\nDE", max_size = 3, na.value = 0.5) +
  scale_shape_discrete(name = "LSC marker\n(Hanekamp 2017)") +
  xlab("Counts in healthy individuals") +
  ylab("Counts in leukemic individuals")
