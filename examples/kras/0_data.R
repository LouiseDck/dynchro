...
write_sce_parts <- function(sce, basefile, colData = TRUE) {
  ...
  if (colData) {
    ...
    colDataSce_rm <- within(
      colDataSce,
      rm("scores", "slingshot", "tradeSeq", "crv")
    )
  ...
  fwrite(
    list(counts_triples$i),
    sep = ",",
    file = glue("{basefile}_counts_triples_i.csv")
  )
  fwrite(
    list(counts_triples$j),
    sep = ",",
    file = glue("{basefile}_counts_triples_j.csv")
  )
  fwrite(
    list(counts_triples$x),
    sep = ",",
    file = glue("{basefile}_counts_triples_x.csv")
  )
...
df <- colData(kras)[, 1:98] %>% as.data.frame() %>% sample_frac(1)
...
p4 <- ggplot(df, aes(x = tSNE1, y = tSNE2, col = Batch)) +
  ...
  scale_color_brewer(palette = "Accent") +
  labs(col = "Pseudotime") +
  geom_path(data = slingCurves(kras, as.df = TRUE), col = "black", size = 1.5)
p4
...
df <- full_join(
  df %>%
    select(X1, tSNE1, tSNE2, Cluster, Batch) %>%
...
for (batch in unique(kras$Batch)) {
  ...
  for (i in 1:3) {
    p4 <- p4 +
      geom_path(
        data = slingCurves(sds_cond)[[i]]$s[
          slingCurves(sds_cond)[[i]]$ord,
        ] %>%
          as.data.frame() %>%
          mutate(Batch = batch),
        size = 1.5
...
for (batch in unique(kras$Batch)) {
  ...
  edges_batch <- left_join(edges, cl_batch, by = c("from" = "Cluster")) %>%
    left_join(
      cl_batch %>%
        dplyr::rename("tSNE1_end" = "tSNE1", "tSNE2_end" = "tSNE2") %>%
        select(-Batch),
      by = c("to" = "Cluster")
    )
  p <- p +
    geom_segment(
      data = edges_batch,
      aes(xend = tSNE1_end, yend = tSNE2_end),
      size = 2
    )
}
p
...
p <- ggplot(df_w, aes(x = Lineage1, y = Lineage3)) +
  ...
  labs(
    x = "Weights for Lineage 1 (w1)",
    y = "Weights for Lineage 2 (w2)",
...
BPPARAM$workers <- 4
icMat <- evaluateK(
  counts = as.matrix(assays(kras)$counts),
...
conditionGenes_lineage1 <- conditionGenes_lineage1[
  !is.na(conditionGenes_lineage1)
]

yhatSmooth <- predictSmooth(
  kras,
  gene = conditionGenes_lineage1,
  nPoints = 50,
  tidy = FALSE
) %>%
  log1p()
yhatSmoothScaled <- t(apply(
  yhatSmooth[, c(1:50, 151:200, 301:350)],
  1,
  scales::rescale
))
heatSmooth_H2122A <- pheatmap(
  yhatSmoothScaled[, 1:50],
  cluster_cols = FALSE,
  show_rownames = FALSE,
  show_colnames = FALSE,
  main = "H2122A",
...
matchingHeatmap_H358A <- pheatmap(
  yhatSmoothScaled[heatSmooth_H2122A$tree_row$order, 51:100],
  cluster_cols = FALSE,
  cluster_rows = FALSE,
  show_rownames = FALSE,
  show_colnames = FALSE,
...
matchingHeatmap_SW1573A <- pheatmap(
  yhatSmoothScaled[heatSmooth_H2122A$tree_row$order, 101:150],
  cluster_cols = FALSE,
  cluster_rows = FALSE,
  show_rownames = FALSE,
  show_colnames = FALSE,
...
p9 <- plot_grid(
  heatSmooth_H2122A[[4]],
  matchingHeatmap_H358A[[4]],
  matchingHeatmap_SW1573A[[4]],
  NULL,
  NULL,
  NULL,
  ncol = 3,
...
conditionGenes_lineage2 <- conditionGenes_lineage1[
  !is.na(conditionGenes_lineage2)
]

yhatSmooth <- predictSmooth(
  kras,
  gene = conditionGenes_lineage2,
  nPoints = 50,
  tidy = FALSE
) %>%
  log1p()
yhatSmoothScaled <- t(apply(
  yhatSmooth[, c(51:100, 201:250, 351:400)],
  1,
  scales::rescale
))
...
conditionGenes_lineage3 <- conditionGenes_lineage3[
  !is.na(conditionGenes_lineage3)
]

yhatSmooth <- predictSmooth(
  kras,
  gene = conditionGenes_lineage3,
  nPoints = 50,
  tidy = FALSE
) %>%
  log1p()
yhatSmoothScaled <- t(apply(
  yhatSmooth[, c(101:150, 251:300, 401:450)],
  1,
  scales::rescale
))
heatSmooth_H2122A <- pheatmap(
